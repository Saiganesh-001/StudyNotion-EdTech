...
function Home() {
	const [CatalogPageData, setCatalogPageData] =
		useState(null);
	const categoryID = "670d3b02536c42fd1028ee11";

	useEffect(() => {
		const fetchCatalogPageData = async () => {
			console.log("page data",categoryID);
			const result = await getCatalogaPageData(
				categoryID,
				dispatch
			);
		...
		<div>
			<div className=" mx-auto relative flex flex-col w-11/12 items-center justify-between text-white ">
				...
				<div className=" mt-4 w-[90%] text-left md:text-center text-sm md:text-lg font-bold text-richblack-300">
					...
					resources, including hands-on projects,
					quizzes, and personalized feedback from
					instructors.
				</div>
					...
					<div className="flex flex-row gap-5 mb-10 mt-[95px]">
						...
						<div className="flex flex-col gap-10 w-[40%] items-start">
							<div className="text-[16px]">
								The modern StudyNotion is the
								dictates its own terms. Today, to
								be a competitive specialist
								requires more than professional
								skills.
							</div>
							<CTAButton
								active={true}
